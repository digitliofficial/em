/**
 * This script runs client-side (in the browser console) and operates directly on in-memory data using `window.em`.
 * It then dispatches `updateThoughts` to sync.
 */

/**
 * Recursively traverses the `thoughtIndex` starting with the root and performs several automatic fixes on all reachable descendants
 * (with the corresponding context in `lexemeIndex` as needed):
 *
 * - missing lexeme
 * - missing rank
 * - missing id
